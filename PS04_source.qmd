...
-   The relationship in @fig-TSG between Trump support and percentage of white households in poverty appears to be **positive**. This relationship seems to be **reasonably linear**.
:::
...
-   No, this will not cause an increase in Trump support. The main reason for this is that **correlation, or association, does not mean causation**. Just because two variables are correlated does not mean they cause increases and decreases in each other.
:::
...
-   Using this prediction model, we can expect Trump support around `r round(predict(poverty_mod, newdata = data.frame(poverty = 6)),2)`%.
:::
...
-   No, it would not be a good idea to use this regression equation to predict Trump support at 85% white poverty. This is because by using the equation we get a percentage over 200%, which doesn't make any sense. This is known as extrapolation and the data doesn't include a data point with white poverty levels over 20%.
:::
...
ggplot(data = trump, aes(y = trump_support, x = non_white)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Percentage of nonwhite households", 
       y = "Percentage of voters supporting Trump", 
       title = "Trump support and nonwhite households in the US") +
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5))
```
:::
...
-   I expect that the correlation coefficient is going to be negative because the slope of the linear model is negative.
...
kable(get_regression_table(nw_mod))
```

-   The intercept means when the percentage of nonwhite households in the area is zero, you can predict a 53.33% Trump support rate.

-   The slope estimates that the percentage of Trump support in an area drops by a value of 0.269 percentage points when the percentage of nonwhite households increases by 1 percentage point.
:::
...
# or 
y_hat = 58.328 - 0.269 * 70
y_hat
```

-   The mathematical prediction using the linear model `nw_mod` shows that if 70% of an area's population was nonwhite, the expected Trump support in that region would be around `r round(predict(nw_mod, newdata = data.frame(non_white = 70)),1)`%.
:::
...
-   The explanatory variable `poverty` seems to be a better explanatory variable of Trump support. This is because the absolute value of its correlation coefficient is larger.
:::
...
**A.** Her prediction of Trump support in that region would be around `r round(predict(nw_mod, newdata = data.frame(non_white = 61)),1)`%.

```{r}
# Type your code and comments inside the code chunk
get_regression_points(nw_mod) |>
  filter(non_white == 61) |>
  select(residual) |>
  kable()
get_regression_points(nw_mod) |>
...
**B.** This prediction would be an overestimate because of a negative residual value. It is an overestimate of 8.912%.

**C.** In other words the residual for this prediction is -8.912.
:::
